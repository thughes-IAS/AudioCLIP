...
#         return logits_audio_text
    ...
    def obtain_audio_embeddings(self, audio):
        ((audio_features, _, _), _), _ = self.aclp(audio=audio)
        audio_features = audio_features / torch.linalg.norm(
            audio_features, dim=-1, keepdim=True)
...
#         logits_audio = scale_audio * audio_features # @ text_features.T
    ...
    def __call__(self, input_dir=None, verbose=False, faiss_index=True, **kwargs):
        ...
        if faiss_index == True:
            ...
            for audio, paths_to_audio in self.preprocess_audio(audio_dir,
                                                           verbose=verbose,
                                                           **kwargs):
                logits_audio = self.obtain_audio_embeddings(audio)
                self.score_inputs_faiss(logits_audio, faiss_index_, paths_to_audio)
        else:
